...
interface DivisionalFooterProps {
  ...
  description: string;
  linksTitle: string;
  /** hrefs should be site-root-relative: '/services', '/contact', etc. */
    ...
    <footer className="bg-[#0f2a37] text-white pt-24 pb-12">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">
          ...
          <div>
            <h4 className="font-bold mb-8 text-sm uppercase tracking-widest text-gray-500">{linksTitle}</h4>
            <ul className="space-y-4 text-gray-400 font-light">
              {links.map((link, idx) => (
                <li key={idx}>
                  <Link href={l(link.href)} className="hover:text-white transition-colors">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            <h4 className="font-bold mb-8 text-sm uppercase tracking-widest text-gray-500">Social</h4>
        ...
        <div className="border-t border-white/5 pt-12 flex flex-col md:flex-row justify-between items-center text-xs text-gray-500 gap-4">
          ...
          <p>Eccellenza Professionale dal 2013</p>
        </div>
      </div>
    </footer>
  );
...
export default DivisionalFooter;
